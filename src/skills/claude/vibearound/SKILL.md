...
Also read `$VIBEAROUND_PROFILE_ID` from the environment if present. If it is empty, `direct`, `default`, `none`, or `off`, omit it.
...
Arguments:
  ...
  profile_id: "<VIBEAROUND_PROFILE_ID if present and not direct/default>"  (optional)
```
